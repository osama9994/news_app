...
class _SearchPageState extends State<SearchPage> {
  ...
  void _doSearch(BuildContext context) {
    ...
    if (text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.tr.text('pleaseEnterSearchTerm'))),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<LanguageCubit, LanguageState>(
      listener: (context, _) =>
          context.read<SearchCubit>().applyCurrentLanguage(),
      child: Scaffold(
        appBar: AppBar(
          title: Text(tr.text('search')),
          centerTitle: true,
        ...
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              _buildCategoryList(context, tr),
              const SizedBox(height: 16),
              Expanded(child: _buildResults(context, tr)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildSearchField(BuildContext context, dynamic tr) {
    return TextField(
      controller: _searchController,
      textInputAction: TextInputAction.search,
      onSubmitted: (value) {
  ...
  Widget _buildCategoryList(BuildContext context, dynamic tr) {
    return SizedBox(
      height: 40,
      child: ListView.separated(
        scrollDirection: Axis.horizontal,
  ...
  Widget _buildResults(BuildContext context, dynamic tr) {
    return BlocBuilder<SearchCubit, SearchState>(
      buildWhen: (_, c) =>
          c is Searching ||
          c is SearchResultsLoaded ||
          c is SearchResultError,
      builder: (context, state) {
        if (state is Searching) return const InterestsShimmer();
        if (state is SearchResultsLoaded) return _buildLoaded(context, state, tr);
        if (state is SearchResultError) return _buildError(context, tr);
        return _buildInitial(context, tr);
      },
    );
  }

  Widget _buildLoaded(
      BuildContext context, SearchResultsLoaded state, dynamic tr) {
  ...
  Widget _buildError(BuildContext context, dynamic tr) {
    return EmptyStateWidget(
      icon: Icons.search_off,
      title: tr.text('noArticlesFound'),
      buttonText: tr.text('retrySearch'),
  ...
  Widget _buildInitial(BuildContext context, dynamic tr) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
